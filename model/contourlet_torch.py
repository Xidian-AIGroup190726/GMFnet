...
def lap_filter(device, dtype, fname="maxflat"):
    ...
    if fname == "9-7":
        ...
        h = np.hstack((h, h[len(h) - 2::-1]))

        g = np.array([-.125 * k1 * k2 * k3, 0.25 * k1 * k2, -0.5 * k1 - 0.5 * k3 - 0.375 * k1 * k2 * k3,
                      1 + .5 * k1 * k2]) * M2
        g = np.hstack((g, g[len(g) - 2::-1]))
        # Normalize
        h = h * np.sqrt(2)
        g = g * np.sqrt(2)
    else:
        raise ValueError("The provided fname is incorrect")
...
def oral_test():
    ...
    im = cv2.imread("contourlet/image/zoneplate.png")
    ...
    print("Space test shape is ", test.shape)
    test = DepthToSpace(w_factor=1)(test)
    print("Depth test shape is ", test.shape)
    print(torch.equal(test, im))

    tic = time()
    nlevs = 3
    xlo, xhi = ContourDec(nlevs)(im)
    print(len(xlo), xlo[0].shape)
...
def test():
    ...
    # img = np.concatenate([img] * 5, axis=0)
    img = np.expand_dims(img, axis=0)
    img = np.concatenate([img] * 200, axis=0)
    img = np.transpose(img, [0, 3, 1, 2])
    print(img.shape)
    norm = 255
    img = np.float32(img) / norm
    img = torch.from_numpy(img).type(torch.FloatTensor)

    # test = SpaceToDepth(w_factor=1)(img)
    # print("Space test shape is ", test.shape)
    # test = DepthToSpace(w_factor=1)(test)
    # print("Depth test shape is ", test.shape)
    # print(torch.equal(test, img))

    tic = time()
    nlevs = 3
    xlo, xhi = ContourDec(nlevs)(img)
    print(time() - tic)
    print(xlo.shape)
    ...
    # import matplotlib.pyplot as plt
    # fig, axs = plt.subplots(nrows=2, ncols=4, figsize=(64, 64))
    # axs = axs.flatten()
    # for j, ax in enumerate(axs):
    #     print(j)
    #     ax.imshow(xhi[j][0, 0, :, :], cmap="gray")
    #     ax.axis('off')
    # plt.show()
